...
function rank(r){return [r.check.errors,r.check.crossings,r.routing.portBends,r.routing.bends,r.check.warnings,r.routing.length];}
...
try{for(const row of rows){const dir=path.join(output,row.name),ref=parseSpice(fs.readFileSync(row.source,'utf8')),before=row.selected;let best=evaluate(path.join(dir,before),ref,before,row.name),initial=best;if(!best.eligible||best.check.errors)throw new Error('Invalid initial '+row.name);
 ...
 log.push({name:row.name,attempts,before,after:best.id,beforeRank:rank(initial),afterRank:rank(best)});fs.writeFileSync(path.join(output,'summary.json'),JSON.stringify(rows,null,2));fs.writeFileSync(path.join(output,'LOOP.json'),JSON.stringify({loop,rank:['errors','crossings','portBends','bends','warnings','length'],rows:log},null,2));console.log(row.name,attempts,before===best.id?'unchanged':'IMPROVED',rank(initial),'->',rank(best));
