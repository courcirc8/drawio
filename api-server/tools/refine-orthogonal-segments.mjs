...
import {orthogonalSegmentProposals,applyTeeProposal} from '../lib/route.js';
import {exportDocument,closeBrowser} from '../lib/render.js';
const [input,output]=process.argv.slice(2);
if(!input||!output||path.resolve(input)===path.resolve(output))throw new Error('Usage: refine-orthogonal-segments.mjs INPUT OUTPUT (distinct directories)');
...
try{for(const row of rows){
 ...
 row.orthogonalPrevious=before;row.improved=improvesOrientation(best,row.results[0]);
