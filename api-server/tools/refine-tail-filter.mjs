...
import {tailFilters,tailFilterCandidate} from '../lib/tail-filter-placement.js';
import {exportDocument,closeBrowser} from '../lib/render.js';
const [input,output]=process.argv.slice(2);
if(!input||!output||path.resolve(input)===path.resolve(output))throw new Error('Usage: refine-tail-filter.mjs INPUT OUTPUT (distinct directories)');
...
try{for(const row of rows){
 ...
 for(let round=1;round<=2;round++){
  ...
  for(const pattern of tailFilters(ref))for(const offset of [120,160,220])for(const gap of [40,70,100]){
   if(Date.now()>=deadline)break;
   const id=`${path.basename(output)}-tail-${round}-${pattern.inductor}-${offset}-${gap}`,p=path.join(dir,id);
   try{
    fs.writeFileSync(p+'.xml',await tailFilterCandidate(xml,pattern,offset,gap));
    const r={...evaluate(p,ref,id),parent,tailFilter:{pattern,offset,gap}};
 ...
 row.tailPrevious=before;row.improved=improvesOrientation(best,row.results[0]);
