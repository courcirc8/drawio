...
import {nearbyTeeProposals,applyTeeProposal} from '../lib/route.js';
import {exportDocument,closeBrowser} from '../lib/render.js';
const [input,output]=process.argv.slice(2);
if(!input||!output||path.resolve(input)===path.resolve(output))throw new Error('Usage: refine-nearby-tees.mjs INPUT OUTPUT (distinct directories)');
...
try{for(const row of rows){
 ...
 for(let round=1;round<=2;round++){
  ...
  for(const proposal of nearbyTeeProposals(getPage(parseDrawio(xml))).slice(0,100)){
   if(Date.now()>=deadline)break;
   const id=`${path.basename(output)}-tee-${round}-${proposal.id}`,p=path.join(dir,id);
   try{
 ...
 row.teePrevious=before;row.improved=improvesOrientation(best,row.results[0]);
