...
if(!input||!output||path.resolve(input)===path.resolve(output))throw new Error('Usage: refine-transistor-orientation.mjs INPUT OUTPUT (distinct directories)');
...
try{for(const row of rows){
 ...
 row.mosPrevious=before;row.improved=improvesOrientation(best,row.results[0]);
