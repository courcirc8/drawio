...
import {spacedMirrorCandidate} from '../lib/transistor-orientation.js';
import {detectStructures} from '../lib/patterns.js';
import {exportDocument,closeBrowser} from '../lib/render.js';
const [input,output]=process.argv.slice(2);
if(!input||!output||path.resolve(input)===path.resolve(output))throw new Error('Usage: refine-spaced-mirrors.mjs INPUT OUTPUT (distinct directories)');
...
try{for(const row of rows){
 ...
 row.spacedPrevious=before;row.improved=improvesOrientation(best,row.results[0]);
