...
Alignment follow-up: owner labels are translated inside the same collision-checked proposal as the component, including conduction-axis mirrors. Horizontal passives with an unambiguous pin-height discrepancy of at most 8px from a MOS or vertical conduction component can align on that fixed pin; conflicting proposed offsets are rejected. This removes the stencil-height jogs that previously created ambiguous tees in common-gate and common-source LNA inputs.

Port alignment follow-up: target positions are computed before committing any move. If a single port is blocked, a pair of uncommitted targets is collision-checked together. Both ports move atomically only when their final positions are clear. This resolves mutually blocking input/bias labels without adding a dogleg to an otherwise horizontal signal path.
